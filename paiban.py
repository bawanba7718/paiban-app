...
from webdav3.exceptions import WebDavException  # 新增：捕获具体异常
import html
...
class AgentViewer:
    # 【保持不变】原有类方法（颜色映射、状态计算等）
    def __init__(self):
        self.color_roles = {
            'FFC000': 'C席', 'FFEE79': 'C席', 'E2EFDA': 'C席', '91AADF': 'C席', 
            'D9E1F2': 'C席', 'EF949F': 'B席', 'FADADE': 'B席', '8CDDFA': '休', 
            'FFFF00': '休', 'FFFFFF': 'A席', 'FEE796': 'C席'
        }
        self.seat_colors = {'C席': '#FFC000', 'B席': '#EF949F', 'A席': '#FFFFFF'}
        self.status_icons = {
            '搬砖中': '🛠️', '干饭中': '🍚', '已回家': '🏠', 
            '正在路上': '🚗', '未排班': '❓', '未知班次': '❓'
        }
        self.shift_times = {
            'T1': {'start': time(8, 0), 'end': time(20, 0), 'name': '白班', 'break_start': time(13, 0), 'break_end': time(14, 0)},
            'T2': {'start': time(20, 0), 'end': time(8, 0), 'name': '夜班', 'break_start': None, 'break_end': None},
            'M2': {'start': time(8, 0), 'end': time(17, 0), 'name': '早班', 'break_start': time(14, 0), 'break_end': time(15, 0)},
            'E2': {'start': time(13, 0), 'end': time(22, 0), 'name': '晚班', 'break_start': time(15, 0), 'break_end': time(16, 0)},
            'E3': {'start': time(13, 0), 'end': time(23, 0), 'name': '晚班', 'break_start': time(15, 0), 'break_end': time(17, 0)},
            'M1': {'start': time(7, 0), 'end': time(16, 0), 'name': '早班', 'break_start': time(12, 0), 'break_end': time(13, 0)},
            'D1': {'start': time(9, 0), 'end': time(18, 0), 'name': '白班', 'break_start': time(12, 0), 'break_end': time(13, 0)},
            'D2': {'start': time(10, 0), 'end': time(19, 0), 'name': '白班', 'break_start': time(13, 0), 'break_end': time(14, 0)},
            'D3': {'start': time(11, 0), 'end': time(20, 0), 'name': '白班', 'break_start': time(15, 0), 'break_end': time(16, 0)},
            'E1': {'start': time(12, 0), 'end': time(21, 0), 'name': '晚班', 'break_start': time(15, 0), 'break_end': time(16, 0)},
            'F1': {'start': time(7, 0), 'end': time(13, 0), 'name': '短班', 'break_start': None, 'break_end': None},
            'F2': {'start': time(9, 0), 'end': time(16, 0), 'name': '短班', 'break_start': time(13, 0), 'break_end': time(14, 0)},
            'F3': {'start': time(17, 0), 'end': time(23, 0), 'name': '短班', 'break_start': None, 'break_end': None},
            'H1': {'start': time(7, 0), 'end': time(11, 0), 'name': '半日班', 'break_start': None, 'break_end': None},
            'H2': {'start': time(16, 0), 'end': time(20, 0), 'name': '半日班', 'break_start': None, 'break_end': None},
        }
        ...
        try:
            ...
            for row_idx, row in enumerate(main_sheet.iter_rows(min_row=2, values_only=False), start=2):
                try:
                    ...
                    if (not shift_code or shift_code.strip() in ['', '休', '休息']):
                        continue
                    seat = self.color_roles.get(color_code, 'A席')
                    person_info = {
                        'name': name, 'id': str(row[2].value).strip() if row[2].value else '',
                        'workplace': str(row[0].value).strip() if row[0].value else '',
                        'shift': shift_code, 'color': color_code, 'seat': seat,
                        'status': '', 'status_color': '', 'actual_seat': seat, 'date': target_date
                    }
    ...
    def categorize_by_seat(self, df, check_time=None):
        ...
        for _, person in df.iterrows():
            status, status_color, actual_seat = self.get_work_status(
                person['shift'], person['seat'], person['color'], check_time
            )
        ...
        for cat in result:
            result[cat].sort(key=lambda x: (-status_priority.get(x['status'], -3), self.get_shift_start_time(x['shift'])))
        return result
                ...
                try:
                    if len(person_row) > col_idx:
                        ...
                        schedule_data.append({
                            'date': date_val, 'weekday': ["一", "二", "三", "四", "五", "六", "日"][date_val.weekday()],
                            'shift': shift_name, 'color': color_code, 'seat': seat
                        })
                except Exception as e:
                    continue
...
def download_from_jiananguo():
    try:
        ...
        if not jiananguo_email or not jiananguo_password:
            return False, None, "坚果云认证信息缺失（邮箱或密码未配置）"
        ...
        options = {
            ...
            'webdav_timeout': 30  # 超时时间延长到30秒，避免网络慢导致失败
        }
        
        # 3. 测试连接
        client = Client(options)
        try:
            client.verify = True  # 验证服务器证书
            client.list()  # 尝试列出文件，测试连接是否有效
        except WebDavException as e:
            return False, None, f"连接坚果云失败：{str(e)}（可能是账号密码错误或网络问题）"
        
        # 4. 确认远程文件路径（用户常因路径变更导致失败）
        remote_file = '我的坚果云/排班.xlsx'  # 请确认该路径是否正确
        if remote_file not in client.list():
            return False, None, f"远程文件不存在：{remote_file}（请检查文件名和路径）"
        ...
        try:
            ...
            os.remove(local_file)  # 下载失败时删除临时文件
            return False, None, f"文件下载失败：{str(e)}"
        
        # 6. 检查文件是否有效
        if not os.path.exists(local_file) or os.path.getsize(local_file) < 1024:  # 小于1KB视为无效
            os.remove(local_file)
            return False, None, "下载的文件为空或损坏"
        
        return True, local_file, ""
        
    except Exception as e:
        return False, None, f"下载过程异常：{str(e)}"
...
def create_compact_agent_card(person_info, viewer):
    ...
    if st.session_state.get(name_key, False):
        ...
        st.session_state[name_key] = False
    card_html = f"""
...
def show_personal_schedule(viewer, file_path, name):
    ...
    selected_month = st.date_input(
        "选择月份", today, format="YYYY-MM-DD", key="personal_month_picker"
    ).replace(day=1)
    with st.spinner(f"正在加载 {name} {selected_month.strftime('%Y年%m月')} 的排班数据..."):
        schedule_data = viewer.get_person_month_schedule(file_path, name, selected_month)
...
def main():
    st.set_page_config(page_title="综合组在线坐席", layout="wide", page_icon="📊")
    ...
    while retry_count < max_retries:
        if st.session_state.file_path and os.path.exists(st.session_state.file_path):
            break  # 已存在有效文件，无需下载
        with st.spinner(f"正在加载排班文件（第{retry_count+1}/{max_retries}次尝试）..."):
            ...
            if download_success:
                ...
                retry_count += 1
                st.warning(f"下载失败（{download_message}），将在3秒后重试...")
                t.sleep(3)  # 等待3秒后重试

    # 最终检查文件是否有效
    if not st.session_state.file_path or not os.path.exists(st.session_state.file_path):
        st.error(f"无法加载排班文件，已达最大重试次数。错误原因：{download_message}")
        st.info("请检查：1. 坚果云账号密码是否正确；2. 网络是否通畅；3. 远程文件路径是否为'我的坚果云/排班.xlsx'")
        return

    if st.session_state.view_mode == 'personal' and st.session_state.selected_person:
        show_personal_schedule(viewer, st.session_state.file_path, st.session_state.selected_person)
    else:
        ...
        with col_controls[1]:
            view_date = st.date_input(
                "选择日期", datetime.now(TZ_UTC_8).date(),
        ...
        with col_controls[2]:
            ...
            selected_time_str = st.selectbox(
                "选择时间", hour_options, index=default_idx,
        ...
        if f"schedule_{load_date_key}" not in st.session_state:
            with st.spinner(f"正在加载{load_date.strftime('%Y年%m月%d日')}的坐席数据，请稍候..."):
                schedule_df = viewer.load_schedule_with_colors(st.session_state.file_path, load_date)
